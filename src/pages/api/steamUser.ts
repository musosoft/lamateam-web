...
export const GET: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ avatar: player?.avatarfull }), { status: 200 });
  } catch (error) {
    console.error('Error fetching Steam user data:', error);
    return new Response(JSON.stringify({ error: 'Failed to fetch Steam user data' }), { status: 500 });
  }
};
